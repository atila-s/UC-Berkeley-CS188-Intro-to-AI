...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        emissionModel = busters.getObservationDistribution(noisyDistance) # P(noisyDistance|trueDistance)
        ...
        for legalPos in self.legalPositions:
            if noisyDistance != None: # The ghost is out of the jail
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for ghostPos in self.beliefs:
            self.setGhostPosition(gameState, ghostPos)
            ghostsDistribution = self.getPositionDistribution(gameState)
            for legalPos in self.legalPositions:
                tempBeliefs[legalPos] += self.beliefs[ghostPos] * ghostsDistribution[legalPos]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        self.particles = [self.legalPositions[particleNo % len(self.legalPositions)] for particleNo in range(self.numParticles)]
    ...
    def observe(self, observation, gameState):
        ...
        for particlePos in self.particles:
            if noisyDistance != None:
                trueDistance = util.manhattanDistance(particlePos, pacmanPosition)
                tempCounter[particlePos] += emissionModel[trueDistance]
            else:
                tempCounter[self.getJailPosition()]=1.0

        tempCounter.normalize()

        if tempCounter.totalCount() == 0:
            self.initializeUniformly(gameState)
        else:
            items = sorted(tempCounter.items())
    ...
    def elapseTime(self, gameState):
        ...
        tempList = [self.getPositionDistribution(self.setGhostPosition(gameState, pos)) for pos in self.particles]
        self.particles = [util.sample(distribution) for distribution in tempList]
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        tempList = list(itertools.product(self.legalPositions, repeat=self.numGhosts))
        random.shuffle(tempList)

        self.particles = [tempList[particleNo % len(tempList)] for particleNo in range(self.numParticles)]
    ...
    def observeState(self, gameState):
            ...
            for particleTuple in self.particles:
                particleList = list(particleTuple)
                cellProbability = 1.0
                for ghostIndex in range(self.numGhosts):
                    if noisyDistances[ghostIndex] != None:
                        particlePos = particleList[ghostIndex]
                        trueDistance = util.manhattanDistance(particlePos, pacmanPosition)
                        cellProbability *= emissionModels[ghostIndex][trueDistance]
                    else:
                        particleList[ghostIndex] = self.getJailPosition(ghostIndex)

                tempCounter[tuple(particleList)] += cellProbability
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            for ghostIndex in range(self.numGhosts):
                newParticle[ghostIndex] = util.sample(getPositionDistributionForGhost(gameState, ghostIndex, self.ghostAgents[ghostIndex]))
